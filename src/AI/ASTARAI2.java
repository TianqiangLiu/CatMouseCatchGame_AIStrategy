...
public class ASTARAI2 implements AI{
	...
	public ArrayList<Integer> getResult(Board board) {
		...
		BFSNode bfsNode = getAStar2ResultNode(board);
		if(bfsNode==null) {
			return null;
		}
	...
	public BFSNode getAStar2ResultNode(Board board){
		...
		bfsNode.setAIScore(getNodeScore(bfsNode,cheesesMap));
	...
	public Integer getNodeScore(BFSNode bfsNode,ArrayList<ArrayList<Integer>>cheesesMap) {
		int sum = 0;
		sum = sum+bfsNode.getRoundInteger();
		sum = sum + heuristic1(bfsNode,cheesesMap);
		return sum;
	}
	public Integer heuristic1(BFSNode bfsNode,ArrayList<ArrayList<Integer>>cheesesMap) {
		//distance to the closest cheese
		int distance = 0;
		...
		if (bfs.getRoundInteger()<mousePath.size()-1) {
			for (ArrayList<Integer> arrayList : catLocations) {
				if(arrayList!=null) {
					...
					tembfs.setAIScore(getNodeScore(tembfs,cheesesMap));
					arr.add(tembfs);
				}
			}
		}
